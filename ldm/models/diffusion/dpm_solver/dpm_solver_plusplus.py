...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None, DPMencode=False):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, **model_kwargs)
        else:
            output = model(x, t_input, cond, DPMencode, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous, DPMencode=False):
        """
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, cond=condition, DPMencode=DPMencode)
            else:
                ...
                noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in, DPMencode=DPMencode,).chunk(2)
...
class DPM_Solver:
    ...
    def noise_prediction_fn(self, x, t, DPMencode=False):
        """
        Return the noise prediction model.
        """
        return self.model(x, t, DPMencode=DPMencode)

    def data_prediction_fn(self, x, t, DPMencode=False):
        """
        Return the data prediction model (with corrector).
        """
        noise = self.noise_prediction_fn(x, t, DPMencode=DPMencode)
    ...
    def model_fn(self, x, t, DPMencode=False):
        """
        Convert the model to the noise prediction model or the data prediction model. 
        """
        if self.algorithm_type == "dpmsolver++":
            return self.data_prediction_fn(x, t, DPMencode=DPMencode)
        else:
    ...
    def sample_one_step(self, data, step, steps, order=3, lower_order_final=True, solver_type='dpmsolver',
        ...
        # We do not need to evaluate the final model value.
        if step < steps:
            data['model_prev_list'][-1] = self.model_fn(data['x'], vec_t, DPMencode=DPMencode)
