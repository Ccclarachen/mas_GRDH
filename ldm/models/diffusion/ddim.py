...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      unconditional_conditioning=None, dynamic_threshold=None, encode=False,):
        b, *_, device = *x.shape, x.device

        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_output = self.model.apply_model(x, t, c, encode=encode,)
        else:
            x_in = torch.cat([x] * 2)
            ...
            model_uncond, model_t = self.model.apply_model(x_in, t_in, c_in, encode=encode,).chunk(2)
    ...
    def decode(self, x_latent, cond, t_start, unconditional_guidance_scale=1.0, unconditional_conditioning=None,
               use_original_steps=False, encode=False,):
    ...
    def decode_one_step(self, x_latent, cond, ts, index, unconditional_guidance_scale=1.0, unconditional_conditioning=None,
                        use_original_steps=False, callback=None, encode=False):
